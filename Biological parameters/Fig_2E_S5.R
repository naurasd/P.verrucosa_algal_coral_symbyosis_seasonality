###############################################################
# Combined Pearson/Spearman correlation heatmap with Arial font
# - Row-wise method:
...
# Read Excel file
df <- read_xlsx("Biological Data file Tilstra et al.xlsx", sheet = "bio + env data", na = "NA")

# Define the 3 environmental variables used in this analysis (Temperature, PAR, DIN)

env_vars <- c("Temperature","PAR","DIN")

# Define the biological variables (we use all available biological variables)

bio_vars <- colnames(df)[3:18]
...
###############################################################
# 8. Supplementary Figure S5: Heatmap (Arial) + bio–bio & bio–env rectangles + Spearman rows
###############################################################
...
# Italics were added to Chlorophyll a and nifH (Ct values) in image processing software outside of R (setting labels as italics did not work here)
...
p_2e <- ggplot(cor_df_2e,
  ...
  theme_minimal(base_family = "Arial") +
  theme(
    axis.text.x = element_text(angle = 45, size = 11,hjust = 0, vjust = 0),
...
ggsave("Figure_2E.pdf",
       plot = p_2e, width = 4.5, height = 3.6)

# Italics were added to Chlorophyll a in image processing software outside of R (setting labels as italics did not work here)